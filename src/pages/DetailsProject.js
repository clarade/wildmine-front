...
const DetailsProject = ({ actualUser }) => {
  ...
  const issuesQuery = useQuery(getIssuesByProjectId, {
  ...
  if (issuesQuery.loading)
    return <div className="mx-auto">Charngement ...</div>;
  ...
  return (
    <div className="detail-project-container">
      <div className="flex justify-between mb-4">
        <div>
          ...
          <div className="project-collaborators flex flex-col items-start justify-start">
            <h3 className="mb-4 mr-4">Collaborateurs du projet :</h3>
      ...
      <Carousel>
        {data.getProjectById.images &&
          data.getProjectById.images.map((image) => (
            <CarouselItem>
              <img
                key={image.id}
                className="object-none object-center"
                src={`${process.env.REACT_APP_SERVER_URL}/images/${image.name}`}
                alt="Projet"
              />
            </CarouselItem>
          ))}
      ...
      {!issuesQuery.loading &&
      issuesQuery.data.getIssuesByProjectId[0] &&
      !issuesQuery.error ? (
        <div>
        ...
        <p className="mx-auto text-xl font-bold tracking-wide py-20">
          Aucun ticket associé à ce projet pour le moment
        </p>
      )}
      <div>
        {!showFiveTickets ? (
